...
var nicEditorConfig = bkClass.extend({
    buttons : {
        ...
        'ol' : {
            name : __('Insertar lista numérica'),
            command : 'insertorderedlist',
            tags : ['OL']
        },
        'ul' : 	{
            name : __('Agregar viñetas'),
        ...
        'removeformat' : {
            name : __('Eliminar formato'),
            command : 'removeformat',
            noActive : true
        },
        'indent' : {
            name : __('Agregar sangría'),
            command : 'indent',
            noActive : true
        },
        'outdent' : {
            name : __('Quitar sangría'),
            command : 'outdent',
            noActive : true
        },
        'hr' : {
    ...
    iconsPath : './taller_dd/nicEditorIcons.gif',
...
var nicEditors={
    ...
    elemById:function(C,F){
        var A=[];
        for(X=0; X<F.length; X++){
            A.push(document.getElementById(F[X]));
        }
        for(var B=0;B<A.length;B++){
            nicEditors.editors.push(new nicEditor(C).panelInstance(A[B]))
        }
...
var nicEditor=bkClass.extend({
    ...
    panelInstance:function(B,C){
        ...
        this.setPanel(A);
        A.setAttribute("class","izquierda");
...
var nicEditorInstance = bkClass.extend({
    ...
    construct : function(G,D,C) {
        ...
        if(H || this.options.hasPanel) {
            ...
            this.editorContain = new bkElement("DIV").setStyle(E).addClass('contenedor').appendBefore(G);
            this.editorContain.addId(G.id+"-contenedor");
            
            //@@@@
            this.editorContain.setAttributes({"tabindex":-1});
               
            this.editorContain.onkeypress = function(){                

                id__ = this.id.replace("-contenedor", "");
                id__ = id__.replace("nicEdit-", "");
                
                var color_ = null;
                color_ = colaborador_color;

                Axx = nicEditors.findEditor(id__.replace("-contenedor", ""));
                if(color_ !=  null){
                    Axx.nicCommand("foreColor", color_);
                }else{
                    Axx.nicCommand("foreColor", "#000000");
                }
            }
            ...
            margin: "4px",
            minHeight : newY+"px"
            }).addClass("main").addId(G.id).appendTo(this.editorContain);
            G.setStyle({
            display : 'none'
            });
...
var nicEditorButton=bkClass.extend({
    construct:function(D,A,C,B){
        ...
        this.button=new bkElement("DIV").setStyle({
            ...
            zoom:1,
            cursor:"pointer"
        }).addClass("button").setStyle(this.ne.getIcon(A,C)).addId(this.name).appendTo(this.border);
