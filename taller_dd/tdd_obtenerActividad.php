<?php
    ...
    if(count($_actividad)>0){
        ...
        $string_actividad.= '</tr>';
        $string_actividad.= '<tr>';
        $string_actividad.= '<td class="td1"><input id="modificar_actividad" title="'.$lang_crear_diseno_editar_act.'" name="'.$_actividad[$i]['ac_id_actividad'].'" class="boton_modificar_actividad" type="button" value="" onClick="abrirActividad('.$_actividad[$i]['ac_id_actividad'].','.$orden_etapa.','.$abajo.','.$arriba.');"/></td>';
        $string_actividad.= '<td class="td2"><input id="eliminar_actividad" title="'.$lang_crear_diseno_elim_act.'" name="'.$_actividad[$i]['ac_id_actividad'].'"  class="boton_eliminar_actividad" type="button" value="" onClick="eliminarActividad('.$_actividad[$i]['ac_id_actividad'].','.$id_etapa.','.$orden_etapa.','.$_actividad[$i]['ac_orden'].');"/></td>';
        if($abajo == 1){                
            $string_actividad.= '<td class="td3" align=center><input name="mover_actividad_abajo" title="'.$lang_crear_diseno_admin_abajo.'" class="boton_mover_abajo" type="button" value="" onClick="bajarActividad('.$_actividad[$i]['ac_id_actividad'].','.$id_etapa.','.$orden_etapa.','.$_actividad[$i]['ac_orden'].');"/></td>';
        }else{
            $string_actividad.= '<td class="td3" align=center></td>';
        }
        if($arriba == 1){
            $string_actividad.= '<td class="td4" align=center><input name="mover_actividad_arriba" title="'.$lang_crear_diseno_admin_arriba.'" class="boton_mover_arriba" type="button" value="" onClick="subirActividad('.$_actividad[$i]['ac_id_actividad'].','.$id_etapa.','.$orden_etapa.','.$_actividad[$i]['ac_orden'].');"/></td>';
        }else{
