...
if(isset($_GET['idDiseno'])){
    ...
    $fcd_diseno_tipo                = $_diseno[0]['dd_tipo'];
    
    if(isset($_diseno[0]['dd_escala']) ){ $fcd_diseno_escala = $_diseno[0]['dd_escala']; }
    
    if($fcd_diseno_tipo == 2){
        $ayuda_text = $ayuda_2;
    }
    
    if($_diseno[0]['dd_id_autor'] == $_SESSION["klwn_id_usuario"]) $isIniciador = true;
    
    $colaborador_color = obtenerParticipanteColorFuncion($idDiseno,  $_SESSION["klwn_id_usuario"], $conexion);
}
?>
        ...
        <div id="formulario_crear_diseno">
            ...
            <form id="form_crear_diseno" method="post" action="" accept-charset="UTF-8">
                <div id="caja_form_crear_diseno">                    
                    ...
                    <input id="editar_diseno_2" class="editar_diseno" type="button" onclick="editarDiseno();" value="<?php echo $lang_crear_diseno_editar; ?>" name="editar">
                    ...
                    <table>
                        ...
                        <tr>
                            <td><label><?php echo (($fcd_diseno_tipo == 1)?$lang_crear_nuevo_diseno_obj_curriculares:$lang_crear_nuevo_diseno_result_aprendizaje) . " :"; ?></label></td>
                            <td><textarea tabindex="6" id="fcd_objetivos_curriculares" name="fcd_objetivos_curriculares" rows="3" cols="1"><?php echo $fcd_objetivos_curriculares; ?></textarea>
                                <?php if($fcd_diseno_tipo == 1){ ?> <a class="abrirObjetivos" title="Ver Objetivos Curriculares" onClick="abrirObjetivosCurriculares();"><img src="./taller_dd/img/pdf.png" class="icono_pdfObjCurr"></img></a> <?php } ?>
                            </td>
                        </tr>
                        <tr>
                            <td><label><?php echo (($fcd_diseno_tipo == 1)?$lang_crear_nuevo_diseno_obj_transversales:$lang_crear_nuevo_diseno_estrat_ensenanza) . " :"; ?></label></td>
                        ...
                        <tr>
                            <td><label><?php echo (($fcd_diseno_tipo == 1)?$lang_crear_nuevo_diseno_contenidos:$lang_crear_nuevo_diseno_estrat_evaluacion) . " :"; ?></label></td>
                        ...
                        <tr>
                            <td><label><?php echo $lang_crear_nuevo_tipo_escala . " :"; ?></label></td>
                            <td>
                                    <?php
                                    for ($i = 0; $i < count($_tipo_escala); $i++) {
                                        if ($_tipo_escala[$i]['tesc_id_tipo_escala'] == $fcd_diseno_escala) {
                                            echo  $_tipo_escala[$i]['tesc_nombre_tipo'] ;
                                        } 
                                    }
                                    ?>
                                </td>
...
<div id="descargasEjemplo">
     <a id="descargaZipEjemplo" href="./taller_dd/tdd_crearZipDocEjemplo.php" target="_black"><?php echo $lang_crear_diseno_descarga_zip_ejemplo; ?></a> 
</div>
...
<script type="text/javascript">
    var colaborador_color = '<?php echo $colaborador_color; ?>';
    var estado = 0;
...
//        document.getElementById('ayuda_d').style.left = (posiciones[0]+400-anc)+"px";
    ...
    function moverIzquierdaActividad(idActividad, id_etapa, orden_etapa, actividad_orden){
        $.get('./taller_dd/tdd_moverActividad.php?mover=izquierda&id_actividad='+idActividad+'&actividad_orden='+actividad_orden+'&id_etapa='+id_etapa+'&orden_etapa='+orden_etapa, function(data) {
            cargarEtapas(id_diseno_actual);
            actualizarCambios(id_diseno_actual);
        });
    }
    
    function moverDerechaActividad(idActividad, id_etapa, orden_etapa, actividad_orden){
        $.get('./taller_dd/tdd_moverActividad.php?mover=derecha&id_actividad='+idActividad+'&actividad_orden='+actividad_orden+'&id_etapa='+id_etapa+'&orden_etapa='+orden_etapa, function(data) {
            cargarEtapas(id_diseno_actual);
            actualizarCambios(id_diseno_actual);
        });
    }    
    ...
    function deshabilitarEdicionDiseno(){
        ...
        $('#editar_diseno').attr('value','<?php echo $lang_tdd_cd_editar;?>');
        $('#editar_diseno_2').attr('value','<?php echo $lang_tdd_cd_editar;?>');
    ...
    function habilitarEdicionDiseno(){
        ...
        $('#editar_diseno_2').attr('value','Cancelar');
            ...
            <?php
                foreach ($_ta_diseno as $key => $value) {
                    echo "if (nicEditors.findEditor('".$value."').getContent() == ''|| nicEditors.findEditor('".$value."').getContent()=='<br>'){".
                         "      nicEditors.findEditor('".$value."').setContent('".$ayuda_text['diseno'][$value]."');".
     ...
     $('#editar_dd').click(function(){
       ...
       //se pone el texto de ayuda en el textarea
        ...
        echo "$('#nicEdit-".$value."').focus(function() {".
             "if (nicEditors.findEditor('".$value."').getContent() == '".$ayuda_text['diseno'][$value]."')".
...
//        document.getElementById('dc_texto_comentario').value = ''
        ...
        foreach ($ayuda_text['diseno'] as $key => $value) {
